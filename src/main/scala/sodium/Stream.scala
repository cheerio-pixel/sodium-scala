...
class Stream[A] private (val node: Node, val finalizers: ListBuffer[Listener], val firings: ListBuffer[A]) {
  ...
  //japi
  final def jGate(c: Behavior[java.lang.Boolean]): Stream[A] =
    filterOptional(
      snapshot[java.lang.Boolean, Option[A]](c, (a: A, pred: java.lang.Boolean) => if (pred) Some(a) else None))

  //japi
  def jGate(c: Cell[java.lang.Boolean]): Stream[A] = jGate(c.behavior)
