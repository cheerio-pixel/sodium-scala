...
public class SDateField extends JComponent {
    ...
    public SDateField(Calendar cal) {
        ...
        //cible   final def lift[B, C, D](fn: (A, B, C) => D, b: Cell[B], c: Cell[C]): Cell[D] = {
        //B Cell<Option<Int
        date = year.selectedItem.lift(monthIndex, day.selectedItem,
        	(Optional<Integer> oy,Optional<Integer> om, Optional<Integer> od) -> {
					return oy.isPresent() && om.isPresent() && od.isPresent()
						? new GregorianCalendar(oy.get(), om.get(), od.get())
						: new GregorianCalendar();
				});
        //date = year.selectedItem.lift(monthIndex, day.selectedItem,(a,b,c) -> new GregorianCalendar(); );
    }
